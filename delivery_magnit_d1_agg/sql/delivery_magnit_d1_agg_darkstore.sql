insert into ft_pa_prod.delivery_magnit_d1_agg_darkstore
with base_info as (
    ...
    where 1 = 1
      and created_dt = toDate('{execution_date}')
      and is_cancelled = 0
      and order_source IN ('Own wl2', 'Аптеки Собственные')
      and magnit_id not in (null, '', 'unknown')
      and store_id in (select CODE
                       from dm.v_whs vw
                       where SUBFRMT like '%Даркстор%'
     ...
     money_data_agg as (
         ...
         from base_info
              ...
              (select create_date as event_date,
                      ...
                      net_revenue,
                      coupon_name
               from dm.ft_order_unit_economics_v where event_date = toDate('{execution_date}')) as margin
     ...
     app_launch_flg_tbl as (
         select toString(magnit_id)  as magnit_id,store_id
         from dm_nrt.loyalty_events__nrt
         where 1 = 1
           and event_date = toDate('{execution_date}')
           and event_name in (
             'app_launch'
             )
           and magnit_id not in (null, '', 'unknown') and store_id in (select CODE
                                                                       from dm.v_whs vw
                                                                       where SUBFRMT like '%Даркстор%'
                                                                         and CLOSE_DT is null
                                                                         and OPEN_DT is not null)
     ...
     delivery_tab_stat as (
         select event_date,
                ...
                if(
                        magnit_id in (select magnit_id from app_launch_flg_tbl),
                        1,
                        NULL
                )     as app_launch_flg,
...
select toDate('{execution_date}') as dt,
